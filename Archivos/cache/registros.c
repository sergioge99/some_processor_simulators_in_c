...
struct ReadyReg disp_reg[max_reg];   /* ciclo de ultima actualizacion de cada reg */
...
void pintar(int num, char usado, int valor, char pend_issue) {
    ...
    else if (valor>0 || pend_issue)  printf("%s%2d->%3d  %s", ROJO, num, valor, RESET);
...
void escribirRegistros() {
	...
	for (int j = 0; j< max_reg; j+=8){
		for (int i = j; i<j+8 && i<max_reg; i++) {
