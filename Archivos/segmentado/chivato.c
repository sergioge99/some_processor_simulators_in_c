...
extern IREG etapa_Bin, etapa_Din, etapa_Ain, etapa_Min, etapa_Ein, etapa_Fin;
...
void chivato()
{
  ...
  printir(&etapa_Ain, fpch);
  printir(&etapa_Min, fpch);
  printir(&etapa_Fin, fpch);
  printir(&etapa_Ein, fpch);
  fprintf(fpch,"\n");

  /*
  fprintf(fpch,"%11s\t%11s%c\t%11s\t%11s\n",
  textco[etapa_Bin.co],textco[etapa_Din.co],dosreg,textco[etapa_Ain.co],textco[etapa_Min.co]);
  fprintf(fpch,"escritura en registro(%d)\n",
	etapa_Min.rd);

  fprintf(fpch,"pc(%x) \n", etapa_Bin.pc);

  decodifica_ins(etapa_Bin.pc,&instruccion);
  imprime_ins(&instruccion,fpch);
  */
...
void verir(int ir,FILE *fp)
{
  ...
  switch(ir)
  {
     ...
     case 2: p = &etapa_Din; break;
     case 3: p = &etapa_Ain; break;
     case 4: p = &etapa_Min; break;
     default:p = &etapa_Ein; break;
  }
